...
static int in_out_test(t_vector normal, t_vector *te, t_vector point, t_vector *ve)
{
      t_vector abc[3];
...
return (0);
	...
	return (0);
        ...
        if (in_out_test(data->obj[y].polygonals[h].normal, data->obj[y].polygonals[h].te,
        vectoradd(ray.start, vectorscale(t, ray.target)), data->obj[y].polygonals[h].ve))
 {
       if (t < data->hit.t)
       {
       data->hit.t = t;
	   ...
	   data->hit.point = vectoradd(ray.start, vectorscale(t, ray.target));
       return (1);
       }
   }
   return (0);
