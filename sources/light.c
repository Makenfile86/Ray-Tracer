...
static t_rgb		get_light_intensity(t_data *data)
{
	...
	else if (data->hit.texture.txt_loaded == TRUE)
	{
		...
		color = ppm_texture(data->hit.texture.res.x, data->hit.texture.res.y,
		data->sphere->texture[i].txt_ppm,
		texture_mapping(data->hit, data->hit.obj_name));
		else if (ft_strcmp(data->hit.obj_name, "plane") == 0)
		color = ppm_texture(data->hit.texture.res.x, data->hit.texture.res.y,
		data->plane->texture[i].txt_ppm,
		texture_mapping(data->hit, data->hit.obj_name));
		else 
		color = ppm_texture(data->hit.texture.res.x, data->hit.texture.res.y,
		data->cylinder->texture[i].txt_ppm,
		texture_mapping(data->hit, data->hit.obj_name));
	}
	else 
...
int check_if_in_shadow(t_ray ray, double *dist, int *in_shadow,
t_vector light_pos)
{
